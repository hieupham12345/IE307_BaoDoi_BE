...
async function initDispose({ $instance, $siloContext, }) {
    return async () => {
        ...
        async function _shutdownNextServices(serviceLoadSequences) {
            ...
            const dependendedByAServiceInTheSameBatch = currentServiceLoadSequence.filter((serviceName) => {
                if (currentServiceLoadSequence
                    ...
                    .some((anotherServiceName) => ($instance._initializersStates[anotherServiceName]
            ...
            await Promise.all(currentServiceLoadSequence
                ...
                if ('silosInstances' in initializeState) {
                    const provider = $instance._getServiceProvider($siloContext, serviceName);
                    ...
                    initializeState.dependents = initializeState.dependents.filter(({ silo }) => silo !== $siloContext.index);
